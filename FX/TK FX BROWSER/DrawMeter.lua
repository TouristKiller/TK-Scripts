...
local function loadJSLoudnessMeter(r)
    ...
    -- Check of de meter al geladen is
    ...
    -- Voeg de meter toe als deze nog niet geladen is
...
function M.DrawMeter(r, ctx, config, TRACK, TinyFont)
    ...
    -- Globale variabelen voor peak hold
    ...
    local function getRMSValues()
        ...
        for i = 0, monitor_fx_count - 1 do
            local _, fx_name = r.TrackFX_GetFXName(r.GetMasterTrack(0), 0x1000000 + i, "")
            if fx_name:find("JS: analysis\\loudness_meter") then
                local rms_M = r.TrackFX_GetParamNormalized(r.GetMasterTrack(0), 0x1000000 + i, 20)
                local rms_I = r.TrackFX_GetParamNormalized(r.GetMasterTrack(0), 0x1000000 + i, 21)
                
                local db_M = -60 + (rms_M * 60)
                local db_I = -60 + (rms_I * 60)
        ...
        local function drawMeterSegments(x, y, width, height, db_value, is_rms)
            local draw_list = r.ImGui_GetWindowDrawList(ctx)
            
            if is_rms then
                -- RMS meter in blauw
                local current_pos = x + ((db_value + 60) / 60) * width
                r.ImGui_DrawList_AddRectFilled(draw_list, x, y, current_pos, y + height, 0x0088FFFF)
            else
        ...
        if r.ImGui_Checkbox(ctx, "Show RMS", config.show_rms_meter) then
            config.show_rms_meter = not config.show_rms_meter

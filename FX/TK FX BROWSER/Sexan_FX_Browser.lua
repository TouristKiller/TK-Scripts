...
local ALL_INFOS_FILE                   = script_path .. "/ALL_FX_INFOS.json"
...
local function ParseVST(name, ident)
    ...
    ident = os_name:match("Win") and ident:reverse():match("(.-)\\") or ident:reverse():match("(.-)/")
        ...
        if line:match("Type%d+") then
            ...
            if fx_type == "3" then -- VST
                ...
                if folder_item then
                    ...
                    if item then
                        local id = os_name:match("Win") and item:reverse():match("(.-)\\") or item:reverse():match("(.-)/")
                        if id then
                            -- NEED TO REPLACE WHITESPACES AND DASH WITH LOWER DASH ALSO (HOW ITS IN VST INI FILE)
...
function GetFXTbl()
    ResetTables()
    if r.file_exists(ALL_INFOS_FILE) then
        LoadInfos()
    else
        GenerateFxList()
        DumpInfos()
...
function RefreshAllFXInfos()
    os.remove(ALL_INFOS_FILE)
    return GetAllFXInfos()
end
...
function DumpInfos()
    ...
    local file = io.open(ALL_INFOS_FILE, "w")
    if file then
        local json_str = json.encode(infos)
        file:write(json_str)
        file:close()
    end
end
...
function LoadInfos()
    local file = io.open(ALL_INFOS_FILE, "r")
    if file then
        local content = file:read("*all")
        file:close()
        local infos = json.decode(content)
        CAT = infos.cat
        DEVELOPER_LIST = infos.dev
        PLUGIN_LIST = infos.plugins
        INSTRUMENTS = infos.instruments
        VST_INFO = infos.vst_info
        VST = infos.vst
        VSTi = infos.vsti
        VST3 = infos.vst3
        VST3i = infos.vst3i
        JS_INFO = infos.js_info
        JS = infos.js
        AU = infos.au
        AUi = infos.aui
        CLAP_INFO = infos.clap_info
        CLAP = infos.clap
        CLAPi = infos.clapi
        LV2_INFO = infos.lv2_info
        LV2 = infos.lv2
        LV2i = infos.lv2i
    end
end

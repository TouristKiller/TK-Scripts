-- @description TK FX Tabs Z-Order Test
-- @author TouristKiller
-- @version 1.0.1.1
-- @changelog:
--   + Test build with delayed z-order correction pulses after tab switching
...
local SCRIPT_NAME = "TK FX Tabs Z-Order Test"
local BLOCKED_WINDOW_NAME = "TK FX Tabs Z-Order Test Blocked State"
...
local pending_pair_front_pulse_index = 0
local pending_pair_front_next_time = 0
...
local PAIR_FRONT_PULSE_DELAYS = { 0, 0.08, 0.18, 0.35, 0.6 }
...
local function clear_pair_front_pulses()
  pending_pair_front_until = 0
  pending_pair_front_pulse_index = 0
  pending_pair_front_next_time = 0
end

local function schedule_pair_front_pulses(now)
  pending_pair_front_until = now + 0.85
  pending_pair_front_pulse_index = 1
  pending_pair_front_next_time = now + PAIR_FRONT_PULSE_DELAYS[pending_pair_front_pulse_index]
end

local function process_pending_pair_front()
  if pending_pair_front_until <= 0 then return end
  local now = r.time_precise()
  if now > pending_pair_front_until then clear_pair_front_pulses(); return end
  if window_touch_paused() or topbar_mouse_captured or topbar_drag_active() then return end
  if topbar_popup_active or now < topbar_popup_hold_until then return end
  if pending_pair_front_next_time > 0 and now < pending_pair_front_next_time then return end
  local entry = find_entry_by_key(active_key)
  local pulse_sequence = pending_pair_front_pulse_index > 0
  local pulsed = false
  if entry_blocked_state(entry) then
    pulsed = bring_blocked_pair_to_front()
  else
    pulsed = focus_active_window()
  end
  if not pulsed then return end
  if not pulse_sequence then clear_pair_front_pulses(); return end
  pending_pair_front_pulse_index = pending_pair_front_pulse_index + 1
  local delay = PAIR_FRONT_PULSE_DELAYS[pending_pair_front_pulse_index]
  if delay then
    pending_pair_front_next_time = now + delay
  else
    clear_pair_front_pulses()
  end
end
...
local function activate_entry(entry)
  ...
  schedule_pair_front_pulses(now)
  last_place_time = 0
end

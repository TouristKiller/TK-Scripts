...
local function ensure_settings(app)
  ...
  settings.max_scan_files = math.max(100, math.floor(tonumber(settings.max_scan_files) or defaults.max_scan_files))
...
local function add_project_file(path, seen)
  ...
  seen[path:lower()] = true
  local kind = file_kind(path)
  state.files[#state.files + 1] = { path = path, name = filename(path), kind = kind, ext = extension(path) }
end
...
local function clear_waveform_pending()
...
local function filter_key(settings)
  return table.concat({ settings.search_term or "", tostring(settings.show_audio), tostring(settings.show_midi), tostring(settings.show_video), tostring(settings.show_image), tostring(settings.location_view_mode), tostring(settings.auto_selected_category), tostring(#state.files), state.category_counts_key or "" }, "|")
end
...
local function refresh_filter(settings)
  ...
  if state.selected_file then
    local found = false
    for index, file in ipairs(result) do
      if file.path == state.selected_file.path then state.selected_index = index; found = true; break end
    end
    if not found then state.selected_index = nil; state.selected_file = nil end
  end
end
...
local function update_auto_play(settings)
  if not settings.auto_play_next or not preview_finished(settings) then return end
  local current_path = state.preview_path
  local file, index = next_preview_file(current_path)
  if file then
...
local function insert_file_on_track(app, file, track)
...
local function update_drag(app)
  ...
  if state.dragging_file then
    ...
    if released or not mouse_down then
      if track then
        insert_file_on_track(app, state.dragging_file, track)
...
local function draw_toolbar(app, settings, width)
  ...
  local loc_w = compact and math.max(82, width - auto_w - button_h * 3 - 36) or math.max(130, width - auto_w - button_h * 3 - 36)
  ...
  r.ImGui_SameLine(ctx)
  if r.ImGui_Button(ctx, "R", button_h, button_h) then start_scan(settings) end
...
local function select_file(app, file, index, autoplay)
  if not file then return false end
...
local function handle_file_list_keyboard(app, settings, row_h)
  ...
  if target_index then
    if select_file(app, state.filtered[target_index], target_index, true) then
  ...
  if key_pressed(ctx, "Enter") or key_pressed(ctx, "KeypadEnter") then
    local file = state.selected_file or state.filtered[index or 1]
  ...
  if r.ImGui_BeginChild(ctx, "##media_files", 0, height, 0, child_flags) then
    local ok, err = pcall(function()
      ...
      for index = first, last do
        local file = state.filtered[index]
        if file then draw_file_row(app, file, index, math.max(80, width - 12)) end
      end
...
function M.init(app)
  ...
  if selected_location(settings) ~= "" then start_scan(settings) end
end

function M.update(app)
  local settings = ensure_settings(app)
  if is_project_files_location(selected_location(settings)) and state.project_files_signature ~= project_files_signature() then start_scan(settings) end
  scan_step(settings)
  refresh_filter(settings)
  monitor_transport_link(settings)
  update_auto_play(settings)
...
function M.draw(app)
  ...
  local transport_size = math.min(18, button_h)
  ...
  r.ImGui_SameLine(ctx)
  local link_w = 30
  r.ImGui_PushStyleColor(ctx, r.ImGui_Col_Text(), settings.link_transport and Theme.colors.accent or Theme.colors.text_dim)
  if r.ImGui_Button(ctx, "link##media_link", link_w, transport_size) then
    settings.link_transport = not settings.link_transport
    if app.save_settings then app.save_settings() end
  end
  r.ImGui_PopStyleColor(ctx)
  if r.ImGui_IsItemHovered(ctx) then r.ImGui_SetTooltip(ctx, "Link transport | Right-click options") end
  ...
  r.ImGui_SameLine(ctx)
  local slider_w = math.max(62, ((avail_w or 320) - transport_size * 2 - link_w - 42) / 3)
  ...
  local info = source .. " | " .. tostring(#state.filtered) .. " / " .. tostring(#state.files) .. " media"
  if shown and shown < #state.filtered then info = info .. " | showing " .. tostring(shown) end
  if state.scanning then info = info .. " | scanning" end

...
--   + Control Room: Each processed mode runs its own instance of the downmix JSFX on its own pair of spare master channels, so one monitor can sit on Side while another checks the low band. An instance is parked rather than deleted once nothing listens to it, "Clear idle" in Setup > Monitors takes the ones nobody is using off the master, and the Channel map names every pair. The crossover frequency sits in the same place

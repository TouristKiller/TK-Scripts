...
--   + Control Room: The footer is a fixed two by three grid - DIM, MONO and Setup above, Meter, Reset and the meter settings below - and it is drawn in both views. It used to be skipped entirely once the meter was open, which moved its controls to the top of the panel and left DIM, MONO and Setup unreachable from there. A button that does not apply is disabled rather than dropped, so nothing ever shifts along, and the three columns share the full width so the labels normally fit unabbreviated. Reported by Bogdan

...
--   + Idea Vault: Ideas can be tagged in bulk. Ctrl-click picks out several cards and shift-click takes the run between two of them, a bar appears saying how many are selected, and "Tag..." adds a tag to all of them or takes one off all of them in a single pass. "Select all shown" takes everything the search and the chips are currently showing, which is the fast way to say "all of these are melodic". The edit dialog changes one idea, and someone who has just worked out that forty of them belong together was never going to open it forty times. Requested by blumpy
--   + Idea Vault: A tag can be renamed, merged or removed across the whole vault from the right-click menu on its chip. Renaming it to a name that already exists merges the two, because that is the same operation seen from the other side - "melodic" and "melodics" were never meant to be two piles - and removing it asks first, since it touches every idea carrying it. A filter that is lit on the tag being renamed follows it, so you are never left with a chip filtering on a tag that no longer exists and no way to click it off. The ideas themselves are untouched either way: only their sidecars change
--   + Idea Vault: An "Untagged" chip, showing how many ideas carry no tag at all. It cannot be one of the tag chips because it matches on the absence of all of them, and while working through a vault it is the one that gets used most: click it for the pile that still wants doing, alt-click it to hide that pile and see what is already sorted. Together with bulk tagging it turns retagging a hundred ideas into something you can actually finish
--   + Idea Vault: The description, the tags and now a rating of an idea can be changed after it is captured, through "Edit description, tags and rating" in the right-click menu or the Edit button in the panel below the list. None of it touches a file: those three live in the sidecar beside the template, which is why editing them costs one small write while renaming still moves everything the idea owns. The name is deliberately not in that dialog - it is what every file of the idea is called, and it stays with Rename. Requested by blumpy
--   + Idea Vault: Tags are a row of chips above the list instead of words that only the search box could see. Click one to show just that tag, ctrl-click to ask for two at once, alt-click to leave one out, and click a lit tag again for the whole vault back - the same gestures the Tags module uses, because it is the same idea. The edit dialog offers every tag already in the vault as a button, since at a hundred ideas a tag with a typo in it is an idea that no chip and no search will ever bring back. Commas separate tags and spaces no longer do, so "dark pad" is one tag rather than two - which is what makes a tag specific enough to be worth having; TK_Idea_Capture.lua, the one-key capture action, splits them the same way, so a tag typed there and the same tag typed in the module are one tag and not two. The chip row is capped at two rows and scrolls beyond that, so a vault with forty tags cannot push the list off the panel. Requested by blumpy
--   + Idea Vault: A rating of nought to five stars, clickable in the panel below the list and in the edit dialog, shown on every card that has one. Clicking the star a rating already ends on clears it, because otherwise there is no way back to unrated. It earns its keep through the two things next to it: the list can be sorted by rating, and "Show at least" hides everything below a number of stars - a rating you cannot sort or filter by is decoration. Ideas captured before this have no rating in their sidecar and read as unrated; nothing needs migrating. Requested by blumpy
--   + Idea Vault: Three densities under the "..." button - the list as it was, compact rows at little over half the height, and tiles whose width you set, with the column count following from the panel and the tiles then stretched to fill it exactly. Sorting is there too: by date captured, name, rating, first tag or length, ascending or not - sorting on tags means the first tag an idea carries, since that is the one that can lead when it has four, and the untagged ones land at the bottom either way. A hundred ideas is where a list that only ever showed the newest first stops being a library and starts being a scroll. Requested by blumpy
--   + Project Browser: A project opened from the browser can run your startup script afterwards. REAPER runs Scripts/__startup.lua exactly once, when the application starts, and never again - so everything that file sets up belongs to the project that happened to be open at boot, and every project opened after it lands in a REAPER that is only half furnished. Tick "Run startup script after opening" in the browser's settings and it runs again after each open. It is registered as an action and called, not loaded into Workbench's own Lua state: startup scripts usually install a defer loop, and one installed inside Workbench would die with Workbench and report its errors as a Workbench failure - this way it runs as its own instance, exactly as REAPER runs it at startup. The path defaults to the well-known file and can point at any .lua, there is a Run now button to see what it does before it runs on every open, and inserting a track template never triggers it, since that drops tracks into the project you already have open rather than starting a new session. It is off by default, because a startup script that launches background scripts launches another copy every time
--   + Idea Vault: Deleting an idea now takes every file that belongs to it, not just the three the sidecar names. A capture can leave more behind than a template, one preview and a sidecar: a project with a secondary output format set writes two audio files per render, and REAPER drops a .reapeaks cache beside any preview it has drawn a waveform for - so the file the sidecar happened to record was removed and its twin stayed in the vault, which is why it was usually the mp3 that survived and occasionally the wav. Everything sharing the idea's name goes, and a file that refuses to go because it is still open is retried for a second and then named in the status bar rather than left behind in silence, since a preview stopped half a frame earlier can still be held by REAPER. Reported by TouristKiller
--   + Idea Vault: Renaming sweeps the same way, for the same reason: it moved the template and one preview and left the second audio file and the peaks cache behind under the old name, where nothing in the module could see them again. A name is also considered taken when any file at all still carries it, not only a template or a sidecar - strays from an earlier delete would otherwise let a new capture claim that name, and the capture finds its rendered file by scanning the folder afterwards, so the new idea would have been handed the old audio as its preview
-- v0.8.6
...
--   + Send Studio: When the fader is pushed onto a second line, the row rearranges rather than leaving a hole: the buttons move right against the edge the fader vacated, and the fader takes every pixel left over on the line it landed on. This needed the row to be measured before it is drawn - deciding a break while drawing is enough to wrap a row, but not to lay one out
--   + Send Studio: The volume slider carries the destination track's colour, so a send can be found by the colour of the bus it feeds. The trough is tinted and the grab stays a theme colour, because ImGui draws the dB readout on top of the grab. The readout picks its own colour against the shade that is really there, and the tint stops at the point where a bright, saturated track colour would leave no text in the palette clearing 4.5:1 against it
--   + Send Studio: Both volume steppers sit behind the fader and cannot be separated from it, in the send rows and on the track's own fader alike. They used to straddle it, one on each side, and wrap onto different lines
--   + Send Studio: Rows are spaced tighter than the theme's general spacing. Every gap in a row is paid for thirteen times across it and again for every send below it, so this is where the density is
--   + Send Studio: The Patchbay button moved onto the track fader row and gives up its word for "PB" when the panel gets narrow. The Sends heading took its place at the right of the toolbar, which saves the line it used to occupy above the list; Receives keeps a heading of its own, right-aligned to match, with the rule underneath it rather than stranded above
-- v0.8.2

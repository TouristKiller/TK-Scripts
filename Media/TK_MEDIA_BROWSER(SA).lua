...
local categorizer_path = script_path .. "tk_filename_categorizer.lua"
local categorizer = dofile(categorizer_path)
local category_cache = {}
local cached_cat_counts = {}
local cached_cat_counts_loc = ""
...
local search_filter = {
    ...
    sorted_files = {},
    remembered_sort_column = -1,
    remembered_sort_direction = 0,
    _presorted = false
}
...
local table_reset_id = 0
local table_columns_fitted = false
...
local function save_file_cache_progressive(location, files, start_index)
    if start_index > #files then
        ...
        save_fast_cache(location, files)
...
save_fast_cache = function(location, files)
    local fast_file = cache_dir .. "fast_cache_" .. location:gsub("[^%w]", "_") .. ".lua"
    local f = io.open(fast_file, "w")
    if not f then return false end
    f:write("return {\n")
    for _, entry in ipairs(files) do
        f:write(string.format(
            '{name=%q,is_dir=false,full_path=%q,parent_folder=%q,ext=%q,name_lower=%q},\n',
            entry.name or "",
            entry.full_path or "",
            entry.parent_folder or "",
            entry.ext or "",
            entry.name_lower or string.lower(entry.name or "")
        ))
    end
    f:write("}\n")
    f:close()
...
local function load_fast_cache(location)
    local fast_file = cache_dir .. "fast_cache_" .. location:gsub("[^%w]", "_") .. ".lua"
    local chunk, err = loadfile(fast_file)
    if chunk then
...
local function LoadTextureHiRes(path)
    local cache_key = path .. "##hires"
    if texture_cache[cache_key] then
    ...
    local max_dim = 1024
    local scale = math.min(max_dim / orig_width, max_dim / orig_height, 1.0)
    local target_width = math.floor(orig_width * scale)
    local target_height = math.floor(orig_height * scale)
...
local function sort_files_by_column(files, col_name, sort_direction)
    ...
    local sorted = {}
    for i, file in ipairs(files) do sorted[i] = file end
    if col_name == "name" then
        local sort_keys = {}
        for i, file in ipairs(sorted) do
            sort_keys[file] = file.name_lower or file.name:lower()
        end
        local desc = sort_direction == r.ImGui_SortDirection_Descending()
        ...
        table.sort(sorted, function(a, b)
            local val_a, val_b
            if col_name == "category" then
                local folder_a = a.full_path:match("(.+)[/\\]") or ""
                local folder_b = b.full_path:match("(.+)[/\\]") or ""
                if not category_cache[a.full_path] then
                    category_cache[a.full_path] = categorizer.classify(a.name or "", folder_a)
                end
                if not category_cache[b.full_path] then
                    category_cache[b.full_path] = categorizer.classify(b.name or "", folder_b)
                end
                val_a = category_cache[a.full_path]
                val_b = category_cache[b.full_path]
            else
                local meta_a = cached_metadata[a.full_path] or {}
                local meta_b = cached_metadata[b.full_path] or {}
                if col_name == "type" then
...
local function build_category_counts(files)
    local results, counts = categorizer.classify_files(files)
    for k, v in pairs(results) do
        category_cache[k] = v
    end
    cached_cat_counts = counts
    cached_cat_counts_loc = search_filter.cached_location
    cached_cat_filter = {}
    cached_cat_filter_key = ""
end

local function presort_flat_files(flat)
    if not flat or #flat == 0 then return flat end
    local rcol = search_filter.remembered_sort_column
    local rdir = search_filter.remembered_sort_direction
    local col_name = rcol >= 0 and get_column_name_by_index(rcol) or "name"
    if not col_name then col_name = "name" end
    if col_name == "name" and (rdir == 0 or rdir == r.ImGui_SortDirection_Ascending()) then
        table.sort(flat, function(a, b)
            return (a.name_lower or a.name:lower()) < (b.name_lower or b.name:lower())
        end)
        search_filter.sorted_files = flat
    else
        table.sort(flat, function(a, b)
            return (a.name_lower or a.name:lower()) < (b.name_lower or b.name:lower())
        end)
        local sorted = sort_files_by_column(flat, col_name, rdir)
        search_filter.sorted_files = sorted
    end
    search_filter.filtered_files = flat
    if rcol >= 0 then
        search_filter.last_sort_column = rcol
        search_filter.last_sort_direction = rdir
        search_filter._presorted = true
    else
        search_filter.last_sort_column = -1
        search_filter.last_sort_direction = 0
    end
        ...
        if has_selection then
            ...
            if use_original_speed then
                ...
                if not is_midi_file then
                    ...
                    if ok and rate and rate > 0 then
    ...
    if file_location.current_location ~= "" then
            ...
            if r.ImGui_BeginChild(ctx, "file_list", 0, 0, 1, child_flags) then
                ...
                if file_location.flat_view then
                    ...
                    elseif ui.current_view_mode == "auto" then
                        if file_location.current_location ~= search_filter.cached_location or #search_filter.cached_flat_files == 0 then
                            local fetched = get_flat_file_list(file_location.current_location)
                            presort_flat_files(fetched)
                            search_filter.cached_flat_files = fetched
                            search_filter.cached_location = file_location.current_location
                        end
                        local base = search_filter.cached_flat_files
                        if ui.auto_selected_category ~= "All" then
                            local filter_key = ui.auto_selected_category .. "_" .. tostring(#base)
                            if filter_key == cached_cat_filter_key and #cached_cat_filter > 0 then
                                flat_files = cached_cat_filter
                            else
                                flat_files = {}
                                for _, file in ipairs(base) do
                                    local cat = category_cache[file.full_path]
                                    if not cat then
                                        local folder = file.full_path:match("(.+)[/\\]") or ""
                                        cat = categorizer.classify(file.name or "", folder)
                                        category_cache[file.full_path] = cat
                                    end
                                    if cat == ui.auto_selected_category then
                                        table.insert(flat_files, file)
                                    end
                                end
                                cached_cat_filter = flat_files
                                cached_cat_filter_key = filter_key
                            end
                        else
                            flat_files = base
                        end
                    ...
                    if r.ImGui_BeginTable(ctx, table_id, col_count, table_flags) then
                        ...
                        if ui_settings.visible_columns.type then
                            r.ImGui_TableSetupColumn(ctx, "Type", fit and S or F, fit and 0.8 or 50)
                        end
                        if ui_settings.visible_columns.size then
                            r.ImGui_TableSetupColumn(ctx, "Size", fit and S or F, fit and 1.0 or 70)
                        end
                        if ui_settings.visible_columns.duration then
                            r.ImGui_TableSetupColumn(ctx, "Duration", fit and S or F, fit and 1.0 or 60)
                        end
                        if ui_settings.visible_columns.sample_rate then
                            r.ImGui_TableSetupColumn(ctx, "SR", fit and S or F, fit and 0.8 or 50)
                        end
                        if ui_settings.visible_columns.channels then
                            r.ImGui_TableSetupColumn(ctx, "Ch", fit and S or F, fit and 0.6 or 50)
                        end
                        ...
                        if ui_settings.visible_columns.artist then
                            r.ImGui_TableSetupColumn(ctx, "Artist", fit and S or F, fit and 1.8 or 100)
                        end
                        if ui_settings.visible_columns.album then
                            r.ImGui_TableSetupColumn(ctx, "Album", fit and S or F, fit and 1.8 or 100)
                        end
                        if ui_settings.visible_columns.title then
                            r.ImGui_TableSetupColumn(ctx, "Title", fit and S or F, fit and 2.5 or 150)
                        end
                        if ui_settings.visible_columns.track then
                            r.ImGui_TableSetupColumn(ctx, "Track", fit and S or F, fit and 0.6 or 50)
                        end
                        if ui_settings.visible_columns.year then
                            r.ImGui_TableSetupColumn(ctx, "Year", fit and S or F, fit and 0.6 or 50)
                        end
                        if ui_settings.visible_columns.genre then
                            r.ImGui_TableSetupColumn(ctx, "Genre", fit and S or F, fit and 1.2 or 80)
                        end
                        if ui_settings.visible_columns.comment then
                            r.ImGui_TableSetupColumn(ctx, "Comment", fit and S or F, fit and 2.5 or 150)
                        end
                        if ui_settings.visible_columns.composer then
                            r.ImGui_TableSetupColumn(ctx, "Composer", fit and S or F, fit and 1.8 or 100)
                        end
                        if ui_settings.visible_columns.publisher then
                            r.ImGui_TableSetupColumn(ctx, "Publisher", fit and S or F, fit and 1.8 or 100)
                        end
                        if ui_settings.visible_columns.timesignature then
                            r.ImGui_TableSetupColumn(ctx, "Time Sig", fit and S or F, fit and 1.0 or 60)
                        end
                        if ui_settings.visible_columns.bitrate then
                            r.ImGui_TableSetupColumn(ctx, "Bitrate", fit and S or F, fit and 1.0 or 60)
                        end
                        if ui_settings.visible_columns.bitspersample then
                            r.ImGui_TableSetupColumn(ctx, "Bit Depth", fit and S or F, fit and 1.0 or 70)
                        end
                        if ui_settings.visible_columns.encoder then
                            r.ImGui_TableSetupColumn(ctx, "Encoder", fit and S or F, fit and 1.8 or 100)
                        end
                        if ui_settings.visible_columns.copyright then
                            r.ImGui_TableSetupColumn(ctx, "Copyright", fit and S or F, fit and 2.5 or 150)
                        end
                        if ui_settings.visible_columns.desc then
                            r.ImGui_TableSetupColumn(ctx, "Description", fit and S or F, fit and 2.5 or 150)
                        end
                        if ui_settings.visible_columns.originator then
                            r.ImGui_TableSetupColumn(ctx, "Originator", fit and S or F, fit and 1.8 or 100)
                        end
                        if ui_settings.visible_columns.originatorref then
                            r.ImGui_TableSetupColumn(ctx, "Orig Ref", fit and S or F, fit and 1.8 or 100)
                        end
                        if ui_settings.visible_columns.date then
                            r.ImGui_TableSetupColumn(ctx, "Date", fit and S or F, fit and 1.2 or 80)
                        end
                        if ui_settings.visible_columns.time then
                            r.ImGui_TableSetupColumn(ctx, "Time", fit and S or F, fit and 1.2 or 80)
                        end
                        ...
                        if search_filter.remembered_sort_column >= 0 and r.ImGui_TableSetColumnSortDirection then
                            ...
                            search_filter.remembered_sort_column = -1
                            search_filter.remembered_sort_direction = 0
                        end
                        
                        if r.ImGui_BeginPopupContextItem(ctx, "##table_header_context") then
                            if r.ImGui_MenuItem(ctx, "Fit All Columns to Window") then
                                table_columns_fitted = true
                                table_reset_id = table_reset_id + 1
                                clear_sort_cache()
                            end
                            if r.ImGui_MenuItem(ctx, "Reset to Default Widths") then
                                table_columns_fitted = false
                                table_reset_id = table_reset_id + 1
                                clear_sort_cache()
                            end
                            r.ImGui_EndPopup(ctx)
                        ...
                        if r.ImGui_TableNeedSort and r.ImGui_TableGetColumnSortSpecs then
                            ...
                            if need_sort and has_specs then
                                if search_filter._presorted then
                                    search_filter._presorted = false
                                else
                                ...
                                if #sort_specs_list > 0 then
                                    ...
                                    if #search_filter.sorted_files > 0 and search_filter.last_sort_column == -1 then
                                        search_filter.last_sort_column = column_index
                                        search_filter.last_sort_direction = sort_direction
                                    elseif column_index ~= search_filter.last_sort_column or 
                                       sort_direction ~= search_filter.last_sort_direction then
                                        ...
                                        if col_name == "name" then
                                            local sort_keys = {}
                                            for i, file in ipairs(search_filter.sorted_files) do
                                                sort_keys[file] = file.name_lower or file.name:lower()
                                            end
                                            local desc = sort_direction == r.ImGui_SortDirection_Descending()
                        ...
                        while r.ImGui_ListClipper_Step(ui.list_clipper) do
                            ...
                            for i = display_start + 1, display_end do
                                ...
                                if ui_settings.visible_columns.category then
                                    ...
                                    if cat ~= "other" then
                                        local cat_color = categorizer.get_category_color(cat)
                                        local dl = r.ImGui_GetWindowDrawList(ctx)
                                        local dx, dy = r.ImGui_GetCursorScreenPos(ctx)
                                        r.ImGui_DrawList_AddCircleFilled(dl, dx + 4, dy + 7, 4, cat_color)
                                        r.ImGui_SetCursorPosX(ctx, r.ImGui_GetCursorPosX(ctx) + 12)
                                        r.ImGui_Text(ctx, cat)
                                    end
                                end
                                if ui_settings.visible_columns.type then
                                    r.ImGui_TableNextColumn(ctx)
...
local function loop()
    ...
    if ui.image_viewer_open and ui.image_viewer_path ~= "" then
        if not ui.image_viewer_img or ui.image_viewer_img_path ~= ui.image_viewer_path then
        ...
        local viewer_flags = r.ImGui_WindowFlags_NoCollapse() | r.ImGui_WindowFlags_NoDocking()
        local fname = ui.image_viewer_path:match("([^/\\]+)$") or "Image"
        r.ImGui_SetNextWindowSize(ctx, 800, 600, r.ImGui_Cond_FirstUseEver())
        local v_visible, v_open = r.ImGui_Begin(ctx, fname .. "###image_viewer", true, viewer_flags)
        if v_visible then
            if ui.image_viewer_img then
                ...
                if img_w > 0 and img_h > 0 then
                    ...
                    local dw, dh
                    if img_aspect > view_aspect then
                        dw = avail_w
                        dh = avail_w / img_aspect
                    else
                        dh = avail_h
                        dw = avail_h * img_aspect
                    end
                    local ox = (avail_w - dw) / 2

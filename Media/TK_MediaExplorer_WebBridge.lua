...
local function get_media_explorer_treeview()
  ...
  for i = 1, #child_addresses do
    ...
    if child then
      local class_name = reaper.JS_Window_GetClassName(child)
      if class_name == "SysTreeView32" then
        return child
      end
    end
  end
        ...
        for i = 0, item_count - 1 do
          ...
          if state and (state & 2 == 2) then
            ...
            if text and text ~= "" then
                -- reaper.ShowConsoleMsg("Open Request: " .. text .. "\n")
                
                -- Check if we are in a database
                if current_path:match("^DB:") then
                    -- In databases, assume items WITHOUT extensions are folders
                    if not text:match("%.%w+$") then
                        should_open = true
                    end
                else
                    -- Filesystem mode: Check if it is a real directory
                    -- We iterate through subdirectories to be 100% sure
                    local is_dir = false
                    local idx = 0
                    while true do

...
local function option_signature(options)
  ...
  return table.concat({
    ...
    tostring(options.include_muted == true),
...
local function read_take_content(item, take, spans, overrides, include_muted)
  if not include_muted and r.GetMediaItemInfo_Value(item, "B_MUTE") == 1 then return end
...
local function collect_content(track, options)
  local spans, overrides = {}, {}
  for index = 0, r.CountTrackMediaItems(track) - 1 do
    local item = r.GetTrackMediaItem(track, index)
    local take = item and r.GetActiveTake(item) or nil
    if take and r.TakeIsMIDI(take) then
      read_take_content(item, take, spans, overrides, options and options.include_muted == true)
    end
  end
...
function Timeline.build(reference, options, force)
  ...
  local spans, overrides = collect_content(track, options)
